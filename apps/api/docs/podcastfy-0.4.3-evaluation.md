...
| `content_generator.py` | Default `model_name` `gemini-1.5-pro-latest` → `gemini-2.5-flash` (only hunk) | **Would change the model we generate with.** Since #539 deleted `ScriptGenerationService`, no first-party code constructs `ContentGenerator` or passes `model_name`; the live path (`generate_podcast`, `tasks/podcast_generation.py:377`) pins no LLM model, so this default *is* our model. A bump silently swaps it — re-validate transcript quality before upgrading (#543 removes the question by owning the call) |
