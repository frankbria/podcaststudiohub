// Mock icon data objects for @hugeicons/core-free-icons
const mockIcon = { type: 'svg', data: [] }

export const Cancel01Icon = mockIcon
export const Tick01Icon = mockIcon
export const ArrowDown01Icon = mockIcon
export const ArrowUp01Icon = mockIcon
export const FolderOpenIcon = mockIcon
export const Inbox01Icon = mockIcon
export const InboxIcon = mockIcon
export const FileEditIcon = mockIcon
export const MusicNote01Icon = mockIcon
export const Alert02Icon = mockIcon
export const AlertCircleIcon = mockIcon
export const CheckmarkCircle01Icon = mockIcon
export const LinkIcon = mockIcon
export const Download01Icon = mockIcon
export const Loading03Icon = mockIcon
export const UserCircle02Icon = mockIcon
export const Logout01Icon = mockIcon
export const PencilEdit01Icon = mockIcon
export const Delete02Icon = mockIcon
export const Sun01Icon = mockIcon
export const Moon01Icon = mockIcon
export const Computer01Icon = mockIcon
export const Home01Icon = mockIcon
export const RssIcon = mockIcon
export const Analytics01Icon = mockIcon
export const Chart01Icon = mockIcon
export const SpotifyIcon = mockIcon
export const AppleIcon = mockIcon
export const WebhookIcon = mockIcon
export const Copy01Icon = mockIcon
export const Refresh01Icon = mockIcon
export const GlobeIcon = mockIcon
export const PodcastIcon = mockIcon
export const PlayIcon = mockIcon
export const Calendar01Icon = mockIcon
