...
def upload_to_s3_task(
    ...
    try:
        ...
        if error_code in NON_RETRYABLE_CODES:
            ...
            raise
        # Transient S3 error — retry with exponential backoff
        logger.warning(
            f"S3 upload transient error for {file_path} "
            f"(attempt {self.request.retries + 1}/{self.max_retries + 1}): {e}"
        )
        if self.request.retries >= self.max_retries:
            logger.error(
                f"S3 upload failed after {self.max_retries} retries for {file_path}: {e}"
            )
            return {
                "status": "failed",
                "s3_key": None,
