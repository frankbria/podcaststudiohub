...
def test_engagement_score_excellent(service):
	...
	score, rating = service._calculate_engagement_score(True, 55)
	assert score == 1.0
	assert rating == "excellent"
...
def test_engagement_score_good_banter(service):
	...
	score, rating = service._calculate_engagement_score(True, 30)
	assert score == 0.8
	assert rating == "good"


def test_engagement_score_many_turns_no_banter(service):
	"""No good banter but >30 turns => good (0.6 score)."""
	score, rating = service._calculate_engagement_score(False, 35)
	assert score == 0.6
	assert rating == "good"  # 0.6 falls in "good" range


def test_engagement_score_poor(service):
	"""No good banter and <=30 turns => fair (0.4 at boundary)."""
	score, rating = service._calculate_engagement_score(False, 5)
	assert score == 0.4
