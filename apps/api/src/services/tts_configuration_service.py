...
default management, and tenant isolation via RLS.
"""
...
from ..schemas.tts_configuration import TTSConfigCreate, TTSConfigUpdate
...
async def create_tts_configuration(
	...
	"""
	Create a new TTS configuration for a user.

	If is_default=True, clears the default flag from all other configurations
...
async def update_tts_configuration(
	...
	Args:
		db: Database session
		tts_config: Existing configuration instance
	...
	if update_dict.get("is_default") is True:
		await _clear_default_flag(db, user_id, tenant_id, exclude_id=tts_config.id)
...
async def delete_tts_configuration(
	...
	Args:
		db: Database session
		tts_config: Configuration instance to delete
	"""
...
async def set_default_tts_configuration(
	...
	"""
	Set a TTS configuration as the default, clearing default from all others.
	...
	Args:
		db: Database session
		tts_config: Configuration to set as default
...
async def _clear_default_flag(
	...
	"""
	Clear is_default flag from all configurations for user/tenant.
