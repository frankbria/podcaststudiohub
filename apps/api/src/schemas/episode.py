...
class BatchEpisodeResult(BaseModel):
	"""Result for a single episode in a batch creation."""

	index: int = Field(..., description="Zero-based index in the request array")
	episode_id: Optional[str] = Field(None, description="Created episode UUID (null if failed)")
...
class BatchEpisodeResponse(BaseModel):
	...
	batch_id: str = Field(..., description="Unique ID for this batch operation")
	status: str = Field(..., description="'complete' or 'partial' (partial if some failed)")
	total_episodes: int = Field(..., description="Total episodes submitted")
	created_count: int = Field(..., description="Number of episodes successfully created")
	failed_count: int = Field(..., description="Number of episodes that failed")
